...
#else	//MG_USE_FMOD
	//FSOUND_Init(44100, 0); //doesn't exist anymore since minifmod 1.70
#endif	//MG_USE_FMOD
...
#else	//MG_USE_FMOD
	//if (!FSOUND_Init(44100, 0)) //doesn't exist anymore since minifmod 1.70
#endif	//MG_USE_FMOD
...
void mgSoundFree()
{
	...
	FMUSIC_FreeSong(CurMod);	//no stop needed
	CurMod = NULL;
#endif
}
...
{
	mgSoundFree();
	//FSOUND_Close(); //doesn't exist anymore since minifmod 1.70
}
...
void mgSoundPlay()
{
	if( CurMod )
		...
		FSOUND_Stream_Play( 0, CurStream );
#endif
}
...
void mgSoundLoadModule(char* name)
{
//	mgSoundFree();
#ifdef MG_USE_FMOD
	CurMod = FMUSIC_LoadSong(name);
#else
	CurMod = FMUSIC_LoadSong(name, NULL);
#endif
	mgClose( (uint)name );
}
